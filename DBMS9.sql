use shrusti;
create table employee5(ename varchar(20), sal int(100),e_post char(20));

insert into employee5 values('schema',90,'IO manager'),
('schools',80,'paleaam'),('pallavi',50,'db manager'),('amitng',50,'java developer');

select ename from employee5 where ename regexp '^sc';
select ename from employee5 where ename regexp 'ng$';
select e_post from employee5 where e_post rlike 'io|ea';
select ename from employee5 where ename regexp '[s,d]';
...
SELECT description,TITLE FROM tutorial WHERE MATCH(title,description) AGAINST('LEFT RIGHT' IN NATURAL LANGUAGE MODE);
